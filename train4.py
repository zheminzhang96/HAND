...
def main_worker():
    ...
    optimizer1 = torch.optim.SGD(model.parameters(), lr=0.00002, weight_decay=args.weight_decay)


    #criterion = getattr(criterions, args.criterion)

    #if args.local_rank == 0:
    checkpoint_dir = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'checkpoint_aux4', args.experiment+args.date)
    ...
    for epoch in range(args.start_epoch, args.end_epoch):
        ...
        with torch.autograd.set_detect_anomaly(True, check_nan=True):
            for i, data in enumerate(train_loader):
                ...
                if i % 100 == 0:
                    vutils.save_image(vutils.make_grid(x, nrow=4, normalize=True, scale_each=True), './log_aux3_2/real_samples'+str(epoch)+'.png')
                    vutils.save_image(vutils.make_grid(output, nrow=4, normalize=True, scale_each=True), './log_aux3_2/fake_samples'+str(epoch)+'.png')
                #print("OOD check")
